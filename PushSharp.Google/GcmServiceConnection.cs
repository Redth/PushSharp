...
{
	public class GcmServiceConnectionFactory : IServiceConnectionFactory<GcmNotification, GcmResponse>
	{
		...
		public IServiceConnection<GcmNotification, GcmResponse> Create()
		{
			return new GcmServiceConnection(Configuration);
		}
	}

	public class GcmServiceBroker : ServiceBroker<GcmNotification, GcmResponse>
	{
	...
	public class GcmServiceConnection : IServiceConnection<GcmNotification, GcmResponse>
	{
		public GcmServiceConnection(GcmConfiguration configuration)
		{
			...
			_http.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("PushSharp", "3.0"));
		...
		public async Task<GcmResponse> Send(GcmNotification notification)
		{
			...
			{
				return await ProcessResponseOk(response, notification).ConfigureAwait(false);
			}
			else
			{
				return await ProcessResponseError(response, notification).ConfigureAwait(false);
			}
		}

		private async Task<GcmResponse> ProcessResponseOk(HttpResponseMessage httpResponse, GcmNotification notification)
		{
			...
			foreach (var r in result.Results)
			{
				...
				else if (r.ResponseStatus == GcmResponseStatus.Unavailable)
				{
					// Unavailable
					multicastResult.Failed.Add(singleResultNotification,
						new GcmNotificationException(singleResultNotification, "Unavailable Response Status"));
				}
				...
				{
					multicastResult.Failed.Add(singleResultNotification,
						new GcmNotificationException(singleResultNotification, "Unknown Failure: " + r.ResponseStatus));
				}
		...
		private async Task<GcmResponse> ProcessResponseError(HttpResponseMessage httpResponse, GcmNotification notification)
		{
		...
		private static GcmResponseStatus GetGcmResponseStatus(string str)
		{
