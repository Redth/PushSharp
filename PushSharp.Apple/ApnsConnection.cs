...
namespace PushSharp.Apple
{
	public class ApnsConnection
	{
		...
		private SemaphoreSlim connectingSemaphore = new SemaphoreSlim(1);
		private SemaphoreSlim batchSendSemaphore = new SemaphoreSlim(1);
		private object notificationBatchQueueLock = new object();

		//readonly object connectingLock = new object ();
		private Queue<CompletableApnsNotification> notifications = new Queue<CompletableApnsNotification>();
		private List<SentNotification> sent = new List<SentNotification>();

		private Timer timerBatchWait;
		...
		private byte[] createBatch(List<CompletableApnsNotification> toSend)
		{
		...
		public class CompletableApnsNotification
		{
			...
			private TaskCompletionSource<Exception> completionSource;
		...
		public static void SetSocketKeepAliveValues(Socket socket, int KeepAliveTime, int KeepAliveInterval)
		{
			...
			BitConverter.GetBytes((uint) KeepAliveTime)
				.CopyTo(inOptionValues, System.Runtime.InteropServices.Marshal.SizeOf(dummy));
			BitConverter.GetBytes((uint) KeepAliveInterval)
