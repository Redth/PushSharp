...
{
	public class ServiceBroker<TNotification, TResponse> : IServiceBroker<TNotification, TResponse> where TNotification : INotification where TResponse : IResponse
	{
		...
		public ServiceBroker (IServiceConnectionFactory<TNotification, TResponse> connectionFactory)
		{
			ServiceConnectionFactory = connectionFactory;

			lockWorkers = new object ();
			workers = new List<ServiceWorker<TNotification, TResponse>> ();
			running = false;
		...
		public event NotificationSuccessDelegate<TNotification, TResponse> OnNotificationSucceeded;
		public event NotificationFailureDelegate<TNotification, TResponse> OnNotificationFailed;

		//public bool AutoScale { get; set; }
		//public int AutoScaleMaxSize { get; set; }
		public int ScaleSize { get; private set; }

		public IServiceConnectionFactory<TNotification, TResponse> ServiceConnectionFactory { get; set; }

		BlockingCollection<TNotification> notifications;
		List<ServiceWorker<TNotification, TResponse>> workers;
		object lockWorkers;
		bool running;
		...
		public void ChangeScale (int newScaleSize)
		{
			...
			lock (lockWorkers) {
				...
				// Scale up
				while (workers.Count < ScaleSize) {
					var worker = new ServiceWorker<TNotification, TResponse> (this, ServiceConnectionFactory.Create ());
					workers.Add (worker);
					worker.Start ();
				}
		...
		public void RaiseNotificationSucceeded (TNotification notification, TResponse response)
		{
			var evt = OnNotificationSucceeded;
			if (evt != null)
				evt (notification, response);
		}

		public void RaiseNotificationFailed (TNotification notification, TResponse response, AggregateException exception)
		{
			var evt = OnNotificationFailed;
			if (evt != null)
				evt (notification, response, exception);
		}
	}

	class ServiceWorker<TNotification, TResponse> 
		where TNotification : INotification
		where TResponse : IResponse
	{
		public ServiceWorker (IServiceBroker<TNotification, TResponse> broker, IServiceConnection<TNotification, TResponse> connection)
		{
			Broker = broker;
		...
		public IServiceBroker<TNotification, TResponse> Broker { get; private set; }

		public IServiceConnection<TNotification, TResponse> Connection { get; private set; }
					...
					try
					{
						...
						foreach (var n in Broker.TakeMany())
						{
							...
							var cont = t.ContinueWith(ct =>
							{
								...
								if (ex == null)
									Broker.RaiseNotificationSucceeded(cn, ct.Result);
								else
									Broker.RaiseNotificationFailed(cn, ct.Result, ex);
							});
	...
	class ServiceWorker<TNotification>
		where TNotification : INotification
	{
